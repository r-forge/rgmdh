...
<<>>=
X <- c("x1","x2","x3")
appl <- function(f,x1,x2) cat(x1,f,x2)
applp <- function(x1,x2) appl("*",x1,x2)
...
x %/% 2^n * 10^n + fxn(x%%2^n,n-1)
}
}

dec2bin <- function(x) fxn(x,floor(log2(x)))

# берёт двоичное число и номер разряда. возвращает значение разряда 1 или 0
razrjad <- function(x,n) round(10*(x/(10^n)-floor(x/(10^n))))
